    ...
    def __init__(self, image_dir, teacher_model, img_size=640, device= 'mps' if torch.backends.mps.is_available() else 'cuda'):
        ...
        with torch.no_grad():
            for img_path in self.image_paths:
                ...
                results = self.teacher.model.predict(img)
                self.teacher_preds.append(results[0][0].transpose(0, 1))
...
def train_distillation(
    ...
    for epoch in range(epochs):
        ...
        for images, targets in loader:
            ...
            student_preds = student.model.predict(images)
